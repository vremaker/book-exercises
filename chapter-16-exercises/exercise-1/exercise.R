...
ggplot(data = diamonds_sample) +
  geom_point(mapping = aes(y = price, x= carat, shape = cut))

# Draw a scatter plot for `diamonds_sample` of *`cut`* by `carat`, where each
# point has an aesthetic _size_ based on the diamond's *`price`*
ggplot(data = diamonds_sample) +
  geom_point(mapping = aes(y = cut, x= carat, size = price))

# Try coloring the above plot based on the diamond's price!

ggplot(data = diamonds_sample) +
  geom_point(mapping = aes(y = cut, x= carat, color = price))
...
ggplot(data = diamonds_sample) +
  geom_col(mapping = aes(x=carat, y = price, color = cut, fill= clarity))
